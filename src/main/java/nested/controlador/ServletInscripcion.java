...
public class ServletInscripcion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int idN = Integer.valueOf(request.getParameter("idNest"));
        int opcion = Integer.valueOf(request.getParameter("opcion"));
        
        if (opcion == 1){
            ParticipacionBD.insertarParticipacion(user.getId(), idN);    
        }
        if (opcion == 2){
            ParticipacionBD.eliminarParticipacion(user.getId(), idN);
        }
        
        
        String url = "/ServletSeccionNest?tipo=2";
        RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(url);
        dispatcher.forward(request, response);
        
        
        //Comentario para tener un cambio
