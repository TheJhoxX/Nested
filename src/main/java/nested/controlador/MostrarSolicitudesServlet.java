...
public class MostrarSolicitudesServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int idUsuarioActual = userActual.getId();
            ArrayList<Usuario> solicitudes = new ArrayList<Usuario>();
            ArrayList<Integer> idSolicitudes = SolicitudDB.getSolicitudesById(idUsuarioActual);
            for(int i = 0; i < idSolicitudes.size(); i++){
                solicitudes.add(UsuarioDB.selectUserID(idSolicitudes.get(i)));
            }
